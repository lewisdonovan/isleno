  ...
  const rows = useMemo(() => {
    ...
    if (view === "general") {
      return sortedFiltered.map(k => ({
        key: k.kpi_id,
        label: k.kpi_name,
        values: intervals.map(int => sumInterval(snapsByKpi.get(k.kpi_id) ?? [], int)),
      }));
    }
    ...
    groupMap.forEach((ids, label) => {
      const values = intervals.map(int => {
        ...
        ids.forEach(id => {
          const v = sumInterval(snapsByKpi.get(id) ?? [], int);
    ...
    return out;
  }, [filtered, snapshots, intervals, view, sortedFiltered]);
